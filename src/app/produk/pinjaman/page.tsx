...
import { getLoanSettings } from '@/app/actions';
...
export default function LoansPage() {
  ...
  useEffect(() => {
    getLoanSettings().then(res => setSettings(res));
  }, []);

  if (!settings) {
    return (
      <div style={{ backgroundColor: '#f4f7fa', minHeight: 'calc(100vh - 120px)', padding: '100px 0', textAlign: 'center' }}>
        <p style={{ color: '#666' }}>{lang === 'ID' ? 'Memuat halaman pinjaman...' : 'Loading loans page...'}</p>
      </div>
    );
  }
            ...
            >
              <div>
                ...
                <p style={{ fontSize: '13px', color: '#7f8c8d', lineHeight: '1.4' }}>
                  {lang === 'ID' 
                    ? `Klik di bawah ini untuk mengajukan pinjaman resmi di ${bank.name}.` 
                    : `Click below to apply for an official loan at ${bank.name}.`}
                </p>
              </div>
              ...
              >
                {lang === 'ID' ? 'Ajukan Pinjaman' : 'Apply for Loan'}
              </a>
            </div>
          ))}
