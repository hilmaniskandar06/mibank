...
import { getSavingsSettings } from '@/app/actions';
...
export default function SimpananPage() {
  ...
  useEffect(() => {
    getSavingsSettings().then(res => setSettings(res));
  }, []);

  if (!settings) {
    return (
      <div style={{ backgroundColor: '#f4f7fa', minHeight: 'calc(100vh - 120px)', padding: '100px 0', textAlign: 'center' }}>
        <p style={{ color: '#666' }}>{lang === 'ID' ? 'Memuat halaman simpanan...' : 'Loading savings page...'}</p>
      </div>
    );
  }
            ...
            >
              <div>
                ...
                <div style={{
                  width: '100%',
                  height: '140px',
                  backgroundColor: bank.logo ? '#ffffff' : '#e2e8f0', // White background if logo exists, grey if not
                  borderRadius: '12px',
                  display: 'flex',
                  alignItems: 'center',
                ...
                <p style={{ fontSize: '13px', color: '#7f8c8d', lineHeight: '1.4' }}>
                  {lang === 'ID' 
                    ? `Klik di bawah ini untuk membuka tabungan resmi di ${bank.name}.` 
                    : `Click below to open an official savings account with ${bank.name}.`}
                </p>
              </div>
              ...
              <a 
                ...
                style={{
                  ...
                  marginTop: '20px',
                  padding: '12px',
                  backgroundColor: 'var(--primary-color, #003366)', // Branding primary blue color
                  color: 'white',
                  textDecoration: 'none',
                  borderRadius: '8px',
                  textAlign: 'center',
              ...
              >
                {lang === 'ID' ? 'Buka Rekening' : 'Open Account'}
              </a>
            </div>
          ))}
