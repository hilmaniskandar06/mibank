...
import { getCreditSettings } from '@/app/actions';
...
export default function CreditCardPage() {
  ...
  useEffect(() => {
    getCreditSettings().then(res => setSettings(res));
  }, []);

  if (!settings) {
    return (
      <div style={{ backgroundColor: '#f4f7fa', minHeight: 'calc(100vh - 120px)', padding: '100px 0', textAlign: 'center' }}>
        <p style={{ color: '#666' }}>{lang === 'ID' ? 'Memuat halaman kartu kredit...' : 'Loading credit cards page...'}</p>
      </div>
    );
  }
            ...
            >
              <div>
                ...
                <p style={{ fontSize: '13px', color: '#7f8c8d', lineHeight: '1.4' }}>
                  {lang === 'ID' 
                    ? `Klik di bawah ini untuk melihat pengajuan kartu kredit resmi di ${bank.name}.` 
                    : `Click below to view the official credit card application at ${bank.name}.`}
                </p>
              </div>
